...
import useIsTouchdevice from "./useIsToucheDevice";
...
function useEventListener<K extends keyof AllEventMaps>(
  type: K,
 ...
 * @param {string} obj.activeText - text to display when cursor is in "active mode"
 * @param {string} obj.textClass - text to display when cursor is in "active mode"
 */
function CursorCore({
  clickables = [
  ...
  const coreStyles: CSSProperties = {
    ...
    transition:
      "opacity 0.15s ease-out, height 0.2s ease-out, width 0.2s ease-out",
  };
  ...
  return (
    <>
      ...
      {isActive && <span className={textClass} style={{mixBlendMode: "exclusion"}}>{activeText}</span>}
        <div
          style={{
